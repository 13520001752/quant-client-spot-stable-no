...
//                    larkService.SendSubscriptionEvent(config, LarkService.UserDataSubOnClose);
    ...
    private int handleUserEventOrderUpdate(
        ...
        priceClose = priceAvg.multiply(percentClose).setScale(newScale, RoundingMode.HALF_UP).stripTrailingZeros();
        priceOpen  = priceAvg.multiply(percentOpen).setScale(newScale, RoundingMode.HALF_DOWN).stripTrailingZeros();
